...
public class HotAdapter extends XRecyclerView.Adapter<HotAdapter.MovieViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MovieViewHolder movieViewHolder, int i) {
            ...
            Glide.with(movieViewHolder.itemView.getContext())
                    .load(resultBeanList.get(i+1).imageUrl)
                    .error(R.mipmap.ic_launcher)
                    .apply(RequestOptions.bitmapTransform(new RoundedCorners(20)))
                    .into(movieViewHolder.imagView);
            movieViewHolder.textName.setText(resultBeanList.get(i+1).name);
            movieViewHolder.textScore.setText(resultBeanList.get(i+1).score+"分");
